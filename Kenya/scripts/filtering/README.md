...
- `kenyaAudienceFilterPipeline.py`       Kenya corpus-level filter (v1/v2)
- `runPiecewiseAudienceFilter.py`        Per-piece Kenya runner (v1/v2)
...
```bash
python Kenya/scripts/filtering/runPiecewiseAudienceFilter.py
```
...
```bash
python Kenya/scripts/filtering/kenyaAudienceFilterPipeline.py --mode v2
# or override paths:
python Kenya/scripts/filtering/kenyaAudienceFilterPipeline.py \
  --mode v2 \
...
```bash
python Kenya/scripts/filtering/filterKenyaComments.py --match-mode conservative
python Kenya/scripts/filtering/filterKenyaComments.py --match-mode expanded
```
...
```bash
python Kenya/scripts/filtering/audienceRelevanceFilterPiecewise.py \
...
```bash
python Kenya/scripts/filtering/createKenyaTop4RelevantWorkbook.py
```
...
|---|---|---|
| `v1` (piecewise) | `runPiecewiseAudienceFilter`, `audienceRelevanceFilterPiecewise` | Word-count floor + basic linguistic-signal hints. More permissive. |
| `v2` (piecewise) | same | Stricter. ≥ 5 words + (≥ 8 words OR at least one hint group + meaningful structure). |
| `conservative` (keyword) | `filterKenyaComments` | Case-insensitive exact-phrase matches against the Kenya/Nigeria sheets of the keyword workbook. |
| `expanded` (keyword) | `filterKenyaComments` | Conservative matches + morphological variants (lowercased stems, common plural/verb forms). |
| `conservative_strict` (Nigeria) | `filterKenyaComments` + exclusion list | Conservative keyword matches minus any rows matching `keywords/conservative_excluded_keywords.txt`. |
